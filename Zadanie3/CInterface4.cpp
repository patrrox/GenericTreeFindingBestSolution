...
void CInterface4 :: test(int iNp)	//metoda do testowania (do usuniecia)
{
	cout << "PO WYGENEROWANIU" << endl;
	initialization(iNp);
	printTreeVector();

	cout << "PO SELEKCJI" << endl;
	...
	for (int i = 0; i < tree_vector.size() - 1; i++)
	{
		tree_vector[i]->crossingTree(*tree_vector[i + 1]);
...
void CInterface4:: initialization(int iNp)
{
	CTree *new_tree;
	for (int i = 0; i < iNp; i++)
	{
...
void CInterface4::printTreeVector()
{
	for (int i = 0; i < tree_vector.size(); i++)
	{
...
void CInterface4::evaluation()
{
	...
	for (int i = 0; i < tree_vector.size(); i++)
	{
		...
		if (tree_vector[i]->getEvaluationResult() < 1)													//zmiana 
		{
...
void CInterface4 :: start(int iNp, int iNi)
{
...
void CInterface4 :: crossingAllTree()
{
	vector <CTree*>tree_vector_new;
	...
	while (tree_vector_new.size() != tree_vector.size())
	{
		i_random_crossing = rand() % (2);						//prawdopodobienstwo krzyzowania
		i_random_one = rand() % (tree_vector.size());
		i_random_two = rand() % (tree_vector.size());
...
void CInterface4 :: mutationAllTree()
{
	int i_random_mutation;
	for (int i = 0; i < tree_vector.size(); i++)
	{
		i_random_mutation = rand() % (2);						//prawdopodobienstwo mutowania
